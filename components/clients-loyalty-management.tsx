...
import { SearchIcon, TrophyIcon, CrownIcon, AwardIcon, StarIcon, PlusIcon, MinusIcon } from "lucide-react"
  ...
  const updateClientPoints = (clientId: string, pointsChange: number) => {
    const updatedClients = clients.map((client) => {
      if (client.id === clientId) {
        const newPoints = Math.max(0, (client.loyaltyPoints || 0) + pointsChange)
        return { ...client, loyaltyPoints: newPoints }
      }
      return client
    })
